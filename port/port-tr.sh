...
green='\e[0;32m'
NC='\e[0m'
tr="$(cat ~/log-install.txt | grep -i "Trojan WS" | cut -d: -f2|sed 's/ //g')"
echo -e "Name : Change Port Trojan WS"
echo -e "========================="
echo -e "Change Port    : $tr"
read -p "New Port Trojan WS: " tr2
if [ -z $tr2 ]; then
...
sed -i "s/   - Trojan WS              : $tr/   - Trojan WS              : $tr2/g" /root/log-install.txt
